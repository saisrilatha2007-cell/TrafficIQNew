  ...
  const mat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        col = mix(col, roadClr, max(max(r1, r2), max(r3, max(rv1, rv2))));
...
function applyCityLabel() {
  const el = document.getElementById('cityLabel');
  if (el) el.textContent = S.city;
}
applyCityLabel();
...
window.scrollToBottom = scrollToBottom; /* expose for HTML scroll btn */
...
function likeMessage(msgId, authorName, btnEl) {
  ...
  setDoc(likerDocRef,{likedAt:serverTimestamp(),liker:S.name,msgId,page:'info-passer'})
  .then(()=>Promise.all([
    setDoc(msgLikeCountRef,{likeCount:increment(1),lastUpdated:serverTimestamp()},{merge:true}),
    setDoc(authorDocRef,{authorName,likeCount:increment(1),lastUpdated:serverTimestamp()},{merge:true}),
  ]))
  .then(()=>console.log('[TrafficIQ] ✅ Like saved. msgId:',msgId,'| author:',authorName))
  .catch(err=>{
...
function subscribeLikes(msgId, btnEl) {
  ...
  if (!likeListeners[msgId]) {
    likeListeners[msgId] = {
      unsub: onSnapshot(doc(window.db,'messageLikeCounts',msgId), snap=>{
...
async function sendMsg() {
  ...
  addAlert({ id:newDocRef.id, name:S.name, role:'b', init:S.name.substring(0,2).toUpperCase(), msg:txt, urg, own:true });
  trackUser(S.name);
  updateRanked(detected, loc);

  await setDoc(newDocRef,{
    name:S.name, role:'b', init:S.name.substring(0,2).toUpperCase(),
    msg:txt, type:detected, loc, urg, city:S.city, createdAt:serverTimestamp()
  });

  const leftPanelRef = doc(window.db,'info-passer_leftpanel',loc);
  await setDoc(leftPanelRef,{
    totalAlerts:increment(1), [`${detected}Count`]:increment(1),
    confidence:increment(5), lastUpdated:serverTimestamp(), city:S.city
  },{merge:true});

  inp.value=''; pendingType='blue';
...
const q = query(collection(window.db,S.collection), orderBy('createdAt'));

onSnapshot(q, snapshot=>{
  for (let key in ranked) delete ranked[key];
  document.getElementById('feed').innerHTML='';

  Object.values(likeListeners).forEach(entry=>{ try{entry.unsub?.();}catch(_){} });
  ...
  snapshot.docs.forEach(docSnap=>{
    ...
    addAlert({
      id:docSnap.id, name:data.name||'Unknown', role:data.role||'b',
      init:data.init||(data.name||'?').substring(0,2).toUpperCase(),
      msg:data.msg||'', urg:data.urg||'blue',
      own:data.name===S.name, ts:data.createdAt||null,
    });
    trackUser(data.name);
  ...
  Object.values(counts).forEach(item=>{ ranked[item.type+':'+item.loc]=item; });
  renderRanked();
});